        ...
        <div class="video-player relative flex flex-nowrap gap-5">
            ...
            <div class="video-player-sidebar flex flex-col shrink-0 w-[330px] h-[470px] bg-[#F5F8FA] rounded-[20px] p-5 gap-5 pb-0 overflow-y-scroll no-scrollbar">
                ...
                <div class="flex flex-col gap-3">
                    ...
                    @php
                        $currentVideoId = Route::current()->parameter('courseVideoId'); // ambil id video dari parameter url
                        $isActive = $currentVideoId == $video->id;
                    @endphp

                        {{-- kirim parameter course dan coursevideoId nya --}}
                        <a href="{{ route('front.learning', [$course, 'courseVideoId' => $video->id]) }}">
                            <div class="group p-[12px_16px] flex items-center gap-[10px] {{ $isActive ? 'bg-[#3525B3]' : 'bg-[#F5F8FA]' }}   rounded-full hover:bg-[#3525B3] transition-all duration-300">
                                @if($isActive)
